...
def harrison_ford
  ...
  # Movie.select(:id, :title).joins(:actors).joins(:castings).where(actors: {actors.name => 'Harrison Ford'}).where.not(castings: {castings.ord => 1})
end
def biggest_cast
  # Consider the following:
  #
  ...
  Movie
    .select(:id, :title).joins(:castings)
    .group('movies.id')
...
def directed_by_one_of(them)
  ...
  # actors = Actor.select(:name, :id).where(actors: { name: [them]}).directed_movies

  Movie
    .select(:id, :title)
    .joins(:director)
    .where( Actor.select(:id, :name).join(:movies).where(movies: { name: them})
